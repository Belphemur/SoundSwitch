...
- `IMMNotificationClient` — exactly **five** callbacks and nothing more: `OnDeviceStateChanged`, `OnDeviceAdded`, `OnDeviceRemoved`, `OnDefaultDeviceChanged`, `OnPropertyValueChanged`, plus the three `IUnknown` methods (`QueryInterface`/`AddRef`/`Release`). SoundSwitch implements all five callbacks; there are no additional callbacks to stub out.
...
**Native cleanup/memory contract (new interop must honor it):** `IMMDevice.GetId` returns a caller-owned `LPWSTR` — free it with `CoTaskMemFree`. Every `PROPVARIANT` obtained from `IPropertyStore.GetValue` must be initialized and then cleared with `PropVariantClear` after the value is copied out. A `GetValue` call that returns `S_OK` with `VT_EMPTY` means the property is **absent**, not an error — the `AudioDevice` property read returns the documented fallback (`""`/`false`/`Active`). Add a `SoundSwitch.Audio.Manager.Tests` unit test pinning the `S_OK` + `VT_EMPTY` → missing-property fallback.

**Dependency invariant (hard, Phase-2 acceptance criterion):** `SoundSwitch.Audio.Manager` must **never** take a back-dependency on `SoundSwitch.Common`. The shared types — `AudioDevice` and the canonical `EDataFlow`/`ERole`/`EDeviceState` enums — live in `SoundSwitch.Audio.Manager`; `SoundSwitch.Common`'s `DeviceInfo`/`DeviceFullInfo` reference those enums, so the reference direction is **Common → Manager only**. `AudioSwitcher.InteractWithDevice` must return the Manager-owned `AudioDevice` (not a `Common` type), and the `AudioSwitcher` methods that today build or accept `Common` types (`DeviceInfo`/`DeviceFullInfo`) must stop doing so — the `DeviceInfo(MMDevice)`/`DeviceFullInfo(MMDevice)` construction moves to the app side, built from an `AudioDevice` returned by the library. Any Manager code that still references a `Common` type has that reference removed in Phase 2; otherwise the tree has a `Common → Manager → Common` project cycle and cannot compile.
...
- **Risk:** medium — WASAPI buffer/timing bugs manifest as silence, clicks, or hung jobs (the 30 s `MaxRuntime` + `SemaphoreSlim` watchdog in `PlaySoundJob` bounds failure); format conversion (PCM→float/resample) is now a hard requirement, not optional; COM apartment — the render loop must run on its own STA/worker thread (never blocking `ComThread`) and the first `IAudioClient` use must be on an STA (§3.5); custom-sound format parity (§3.5) is a product decision, not a technical one.
...
**Done criteria:** zero `using NAudio` in the repo; zero NAudio `PackageReference`/`PackageVersion`; all automated gates green; the smoke matrix passes; two releases' worth of Sentry quiet on the audio-service-down signature.
