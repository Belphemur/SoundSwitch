...
**Done criteria:** zero `NAudio` substring matches in the repo (covers both `using NAudio` and qualified `NAudio.*` names); zero NAudio `PackageReference`/`PackageVersion`; all automated gates green; the smoke matrix passes; two releases' worth of Sentry quiet on the audio-service-down signature.
