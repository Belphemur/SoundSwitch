...
- `IAudioClient`, `IAudioRenderClient` — for notification playback (§3.5). `IAudioClient` must declare the **complete ordered vtable** — after the three `IUnknown` slots, in declaration order: `Initialize`, `GetBufferSize`, `GetStreamLatency`, `GetCurrentPadding`, `IsFormatSupported`, `GetMixFormat`, `GetDevicePeriod`, `Start`, `Stop`, `Reset`, `SetEventHandle`, `GetService` — because a COM interface dispatches by slot position: **omitting a slot shifts every subsequent method to the wrong slot**. `IAudioRenderClient` declares `GetBuffer`/`ReleaseBuffer`. `GetCurrentPadding` is required by the §3.5 render fill loop (underrun/overrun safety); `IsFormatSupported`/`GetMixFormat` are required by the §3.5 format-negotiation step.
...
- **Custom-sound parity (a real, user-facing path, not hypothetical):** user-selected custom sounds go through `CachedSound.GetReader` (`CachedSound.cs:83–93`), which tries `new AudioFileReader(filename)` and falls back to `new MediaFoundationReader(filename)` on `MmException` — i.e. custom sounds support non-WAV (MP3/FLAC/AAC) today. Live call sites: `Settings.cs:1004` (`AppModel.Instance.CustomNotificationSound = new CachedSound(selectSoundFileDialog.FileName)`) and `Model/AppModel.NotificationSettings.cs:98` (`new CachedSound(AppConfigs.Configuration.CustomNotificationFilePath)`). The parity options are (a) keep a Media Foundation fallback for non-WAV custom sounds, or (b) scope custom sounds to WAV. **`IMFSourceReader` is a *native* Media Foundation COM interface, not a CsWinRT-projected API** — so option (a) is *not* a "thin CsWinRT fallback"; it requires explicit COM/P-Invoke interop plus Media Foundation lifetime/format handling (or a separate decoder dependency), and it is real, non-trivial work that Phase 4 must budget for. **Recommendation: commit to the native interop cost explicitly if non-WAV parity is required; otherwise accept WAV-only as a *deliberate* user-visible regression and tighten the file-open dialog filter to match — do not call the MF path "thin".** This is the one place the migration can accidentally remove a feature — flag it in the PR and require a `feat`/`fix` framing either way.
...
- **Work:** in-house `WaveFileReader` (RIFF/PCM+IEEE-float) + `WaveFormat` record; `WavePlayer` over `IAudioClient`/`IAudioRenderClient` (shared mode, event-sync, ~200 ms buffer, explicit endpoint or default render/`eMultimedia`, completion event with exception slot). **Format negotiation is mandatory:** `WavePlayer.Initialize` must query `IAudioClient.GetMixFormat` (typically 48 kHz 32-bit IEEE float) and either negotiate via `IsFormatSupported` or perform PCM→float conversion + resampling — a "matching-formats-only" player fails on the default render endpoint for any WAV that isn't already 48 kHz float, i.e. the common case (§3.5). The render fill loop sizes each write via `IAudioClient.GetCurrentPadding` (underrun/overrun safety, §3.5). Reshape `CachedSound` (drops NAudio `WaveFormat`, decodes via the in-house reader; resolve the custom-sound parity decision from §3.5 — `IMFSourceReader` is native Media Foundation COM, **not** a CsWinRT fallback, so non-WAV parity means committing to that native interop cost, otherwise accept and document a deliberate WAV-only regression and tighten the file dialog filter); delete `CachedSoundWaveStream`; rewrite `PlaySoundJob` to call the public §3.3 playback facade (killing its private `MMDeviceEnumerator`); the `WavePlayer` render loop runs on its **own dedicated COM-initialized** STA/worker thread — never blocking the `ComThread` — with the first `IAudioClient` use on an STA (§3.5); touch `NotificationSound`, `BannerData`, `INotificationSettings`, `AppModel.NotificationSettings`, `Settings.cs` L1004 only for the `CachedSound`/`WaveFormat` type change.
...
| **Custom-sound format regression** (`AudioFileReader`/`MediaFoundationReader` accept MP3 etc. today) | Users with non-WAV custom sounds lose playback | Product decision surfaced in the release notes. `IMFSourceReader` is a **native** Media Foundation COM interface, not a CsWinRT projection — keeping non-WAV parity means explicit COM/P-Invoke interop + MF lifetime/format handling (non-trivial); otherwise explicitly scope custom sounds to WAV and tighten the file dialog filter (§3.5). Do not silently drop, and do not call the MF path "thin". |
...
4. `SoundSwitch.Audio.Manager.Tests` — must stay green and grow per phase (WAV parser tests in Phase 4, enum/plumbing tests in Phases 1–2). Note CI sets `CI=true`, so the hardware-dependent tests in `SoundSwitch.Tests` (`RefreshDeviceTests`, `DeviceCyclerTests`) self-skip; they still run on a developer machine with real audio devices and should be run locally before merging Phases 2–4.
...
Folded 18 review threads (CodeRabbit + Copilot) into §1–§6; Appendices A/B retained verbatim.
