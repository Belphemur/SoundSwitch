...
- **Update subsystem usage counts** — anonymous counters for which update mode is configured (`Silent`, `Notify`, or `Never`, emitted when the setting changes and once at startup as a baseline), when a check for updates is triggered (with a categorical `trigger` of `manual` for a tray-menu "Check for update" click, or `setting_change` when a check is kicked off automatically by a settings change such as the beta channel or update mode), when a newer release is found and offered, when an update download fails (`soundswitch.update.download_failed`, a network/storage failure before any install is attempted), and when an install is attempted or applied (with a categorical result of `success` — meaning the installer was launched — `signature_error`, or `failed` — meaning the installer could not be launched). Download (network) failures are reported separately from install failures and are never labeled as an install attempt. No version numbers, file paths, or other identifiers are sent — only the categorical update mode, trigger, and outcome.
