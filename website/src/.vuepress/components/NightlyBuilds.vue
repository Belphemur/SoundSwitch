...
// Changelog links are generated by CI as commit permalinks; anything else in a commit
// subject must never become an anchor (javascript:/data: XSS vector).
...
function changelogTokens(line: string): ChangelogToken[] {
  ...
  while ((match = regex.exec(stripped)) !== null) {
    ...
    if (isSafeHref(match[2])) {
      ...
      tokens.push({ isLink: false, text: match[1] })
      tokens.push({ isLink: false, text: ` (${match[2]})` })
    }
