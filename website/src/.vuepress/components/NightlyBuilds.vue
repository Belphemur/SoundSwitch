...
import { parseChangelogLine, type ChangelogToken } from '../modules/changelog'
...
interface ChangelogEntry {
  tokens: ChangelogToken[]
}

// Lazily parse a nightly's changelog lines into render tokens the first time the
// <details> "spoiler" is opened, so we only do the work when the user asks for it.
function buildChangelog(lines: string[] | undefined): ChangelogEntry[] {
  return (lines ?? []).map((line) => ({ tokens: parseChangelogLine(line) }))
}

const parsedChangelogs = ref<Record<string, ChangelogEntry[]>>({})

function nightlyChangelog(nightly: AvailableNightly): ChangelogEntry[] {
  return parsedChangelogs.value[nightly.id] ?? []
}

function revealChangelog(nightly: AvailableNightly): void {
  if (parsedChangelogs.value[nightly.id]) {
    return
  }
  parsedChangelogs.value = {
    ...parsedChangelogs.value,
    [nightly.id]: buildChangelog(nightly.changelog),
  }
}
    ...
    <ol v-else class="nightly-builds__list">
      <li v-for="nightly in nightlies" :key="nightly.id" class="nightly-builds__item">
        <div class="nightly-builds__content">
          ...
          <details v-if="nightly.changelog && nightly.changelog.length" class="nightly-builds__details" @toggle="revealChangelog(nightly)">
            <summary>What's changed</summary>
            <ul class="nightly-builds__changelog">
              <li v-for="(entry, index) in nightlyChangelog(nightly)" :key="index">
                <template v-for="(token, tokenIndex) in entry.tokens" :key="tokenIndex">
                  <a v-if="token.isLink" :href="token.href" target="_blank" rel="noopener">{{ token.text }}</a>
                  <strong v-else-if="token.isBold">{{ token.text }}</strong>
                  <em v-else-if="token.isItalic">{{ token.text }}</em>
